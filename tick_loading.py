...
if __name__ == '__main__':
    ...
    end = datetime.datetime.strptime("20191022", "%Y%m%d")  # the end date is not included, so +1

    date_generated = [start + datetime.timedelta(days=x) for x in range(0, (end - start).days)]
    n = 1  # marker since we can't create an empty measurement
    ...
    for date in date_generated:
        ...
        pool.map(load_trade, [chunk for chunk in record])
